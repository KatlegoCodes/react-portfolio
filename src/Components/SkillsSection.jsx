...
/**
 * SkillsSection.jsx
 * - Equal-height cards (flex + h-full)
 * - Subtle linears and halos behind icons
 * - Tools section with custom hover color/linear per tool
 */
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 items-stretch">
          {Object.entries(skills).map(([key, category]) => (
            <motion.div
              key={key}
              className="flex flex-col h-full bg-white dark:bg-gray-800 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-gray-200 dark:border-gray-700 p-8 hover:shadow-[0_12px_40px_rgba(15,23,42,0.12)] transition-all duration-300 group"
              initial={{ opacity: 0, y: 30 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <div className="flex items-center mb-6">
                <div className="relative mr-4">
                  <div
                    className={`absolute inset-0 rounded-xl blur-md opacity-20 group-hover:opacity-50 transition ${category.linear}`}
                    style={{ filter: "blur(10px)" }}
                  />
                  <div className="relative p-3 bg-gray-900 dark:bg-gray-700 rounded-xl text-white">
                    {category.icon}
                  </div>
                </div>
              ...
              {/* Skill Bars (flex-grow to align cards) */}
                ...
                {category.technologies.map((skill, index) => (
                  <div key={skill.name} className="group">
                    ...
                    <div className="relative w-full bg-gray-200 dark:bg-gray-700 rounded-full h-3 overflow-hidden shadow-inner">
                      ...
                      <span className="absolute -top-6 right-0 text-sm font-semibold text-gray-600 dark:text-gray-300 opacity-0 group-hover:opacity-100 transition-opacity">
                        {skill.level}%
                      </span>
                    </div>
                  </div>
        ...
        {/* Tools & Technologies (with custom hover styles) */}
        <motion.div
          className="mt-14"
            ...
            {tools.map((tool, idx) => {
              ...
              return (
                <motion.button
                  ...
                  // prevent focus outline being too harsh
                  onMouseDown={(e) => e.preventDefault()}
                >
                  {/* If you want to add small icons later, put them here */}
                  <span>{tool}</span>
                </motion.button>
              );
            })}
        ...
        >
          ...
          <a
            href="#contact"
            className="relative inline-flex items-center px-8 py-3 font-semibold text-white rounded-xl bg-linear-to-r from-indigo-500 to-purple-500 overflow-hidden group transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1"
          >
            <span className="absolute inset-0 bg-linear-to-r from-indigo-600 to-purple-600 opacity-0 group-hover:opacity-100 transition duration-500 blur-lg" />
